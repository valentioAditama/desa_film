...
function uplode(){
    if (isset($_POST['uplode'])) {
        ...
        }else if (!in_array($imageExt, $ekstensionFIle) == !in_array($video, $ekstensionFIlevideo)) {
            echo "<script>alert('error! allowed ekstension image .jpg, .jpeg, .png or .mp4 .wemb .mkv')</script>";
...
function edit(){
        ...
        $error = "";
        if (!in_array($imageExt, $ekstensionFIle) == $error) {
            echo "<script>alert('allowed .jpg .jpeg .png')</script>";
        }
